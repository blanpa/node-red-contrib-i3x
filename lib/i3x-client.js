...
class I3XClient extends EventEmitter {
     ...
     * @param {string}   [options.endTime]   – ISO 8601; defaults to now (the 1.0
     *        spec requires endTime, so an omitted value is filled with the current time)
     * @param {number}   [options.maxDepth]
     */
    async readHistory(elementIds, options = {}) {
        const body = { elementIds };
        if (options.startTime) body.startTime = options.startTime;
        // i3X 1.0 servers require endTime on history queries (400 "endTime: Field
        // required" otherwise). Default an omitted endTime to the current time.
