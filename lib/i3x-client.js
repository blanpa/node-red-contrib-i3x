...
class I3XClient extends EventEmitter {
    /**
     * @param {object} config
     * @param {string} config.baseUrl       – e.g. "https://api.i3x.dev"
    ...
    /** Default history window when no startTime is given: one hour. */
    static DEFAULT_HISTORY_WINDOW_MS = 3600000;

    /**
     * Bulk historical values query (POST).
     *
     * The 1.0 spec marks both `startTime` and `endTime` as required – a live
     * server answers 400 ("startTime: Field required") if either is missing.
     * Omitted values are therefore filled in: `endTime` with the current time,
     * `startTime` with one hour before `endTime`.
     *
     * @param {string[]} elementIds
     * @param {object}   [options]
     * @param {string}   [options.startTime] – ISO 8601; defaults to endTime − 1h
     * @param {string}   [options.endTime]   – ISO 8601; defaults to now
     * @param {number}   [options.maxDepth]
     */
    async readHistory(elementIds, options = {}) {
        const body = { elementIds };
        body.endTime = options.endTime || new Date().toISOString();
        if (options.startTime) {
            body.startTime = options.startTime;
        } else {
            const endMs = Date.parse(body.endTime);
            const baseMs = Number.isNaN(endMs) ? Date.now() : endMs;
            body.startTime = new Date(baseMs - I3XClient.DEFAULT_HISTORY_WINDOW_MS).toISOString();
        }
        if (options.maxDepth !== undefined) body.maxDepth = options.maxDepth;
    ...
    /**
     * Parse one SSE event block (lines between blank lines) into its data payload.
     * Per the SSE spec, comment lines (":" prefix) are ignored, a single leading
     * space is stripped from each field value, and multiple `data:` fields are
     * concatenated with newlines.
     * @param {string} block – event block with LF line endings
     * @returns {string|undefined} the data payload, or undefined if the event carried none
     * @private
     */
    static _parseSSEEvent(block) {
        const data = [];
    ...
    streamSubscription(subscriptionId, callbacks, options = {}) {
        ...
        const connect = async () => {
            ...
            try {
                ...
                reconnectCount = 0;
                // StringDecoder keeps multi-byte UTF-8 sequences intact when a
                // character straddles a chunk boundary.
                const decoder = new StringDecoder("utf8");
                let buffer = "";
                response.data.on("data", (chunk) => {
                    buffer += decoder.write(chunk);
                    // SSE allows LF, CRLF and CR line endings; events are
                    // separated by a blank line. Normalise before splitting.
                    buffer = buffer.replace(/\r\n|\r/g, "\n");
                    const parts = buffer.split("\n\n");
                    buffer = parts.pop();
                    for (const part of parts) {
                        const payload = I3XClient._parseSSEEvent(part);
     ...
     *
     * A HTTP 206 response means the server dropped updates because the
     * subscription queue overflowed. The returned batches then carry a
     * non-enumerable `_overflow` flag so callers can surface the data loss.
     *
     * @param {string} subscriptionId
     * @param {object} [options]
    ...
    async syncSubscription(subscriptionId, options = {}) {
        ...
        const res = await this._requestRaw("post", "/subscriptions/sync", { data: body });
        const result = I3XClient._unwrapEnvelope(res.data);
        if (res.status === 206 && result && typeof result === "object") {
            // Non-enumerable so the flag survives on an array without showing
            // up in JSON.stringify or for..in over the batches.
            Object.defineProperty(result, "_overflow", {
                value: true,
                enumerable: false,
                configurable: true,
            });
